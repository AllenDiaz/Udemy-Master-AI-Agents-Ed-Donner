...
MODEL = os.environ.get("WORKER_MODEL", vertex_client.DEFAULT_MODEL)
...
# Vertex AI serves an OpenAI-compatible endpoint. Agno's OpenAILike is the
# documented choice for any non-OpenAI OpenAI-compatible endpoint: same shape as
# OpenAIChat but with base_url + api_key. The api_key is a fresh ADC token; the
# model id gets a "google/" prefix, which is what Vertex's endpoint expects.
model = OpenAILike(
    id=vertex_client.resolve_model(MODEL),
    base_url=vertex_client.get_base_url(),
    api_key=vertex_client.get_access_token(),
)
