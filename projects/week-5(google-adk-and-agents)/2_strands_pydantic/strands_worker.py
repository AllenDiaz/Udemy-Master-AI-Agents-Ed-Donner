...
# Vertex AI serves an OpenAI-compatible endpoint, so the same OpenAIModel works
# against it. The base URL comes from GOOGLE_CLOUD_PROJECT + GOOGLE_CLOUD_LOCATION,
# the API key is a fresh ADC OAuth token, and the model id gets a "google/" prefix.
model = OpenAIModel(
    client_args={
        "api_key": vertex_client.get_access_token(),
        "base_url": vertex_client.get_base_url(),
    },
    model_id=vertex_client.resolve_model(MODEL),
)
